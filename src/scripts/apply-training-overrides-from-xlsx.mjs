...
function parseWorkbook(fileBuffer) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const taxCode = normalizeTaxCode(r[2]);
...
function mapCourseLabelToCodes(label) {
...
async function main() {
  ...
  const filePathArg = process.argv.find((v) => String(v).toLowerCase().endsWith(".xlsx")) ?? null;
  if (!filePathArg) {
  ...
  const inputRows = parseWorkbook(fileBuffer);
  ...
  const employeesByTaxCode = new Map();
  for (let i = 0; i < taxCodes.length; i += 500) {
    const chunk = taxCodes.slice(i, i + 500);
    const { data, error } = await supabase.from("employees").select("id,tax_code").in("tax_code", chunk);
  ...
  const { data: courses, error: coursesError } = await supabase.from("training_courses").select("id,code,title");
  if (coursesError) throw new Error(coursesError.message);
  const coursesByCode = new Map();
  const coursesByNormalizedTitle = new Map();
  ...
  if (!dryRun && deduped.length > 0) {
    for (let i = 0; i < deduped.length; i += 500) {
      ...
      const { error } = await supabase.from("training_employee_courses").upsert(chunk, { onConflict: "employee_id,course_id" });
      if (error) throw new Error(error.message);
    }
  }
