...
// TEST endpoint — same logic as cron but NO auth required
  ...
  try {
    ...
    if (!folderIdEnv || !serviceAccountJson) {
      return NextResponse.json(
        {
          error: "Config incomplete: GOOGLE_DRIVE_IMPORT_FOLDER_ID or GOOGLE_SERVICE_ACCOUNT_JSON missing",
        },
        { status: 500 }
      );
    }
    ...
    if (files.length === 0) {
      return NextResponse.json(
        { message: "No files found in import folder", folderIdEnv },
        { status: 200 }
      );
    }
    ...
    // 5. Process import in preview mode
    ...
    console.log(`[Test Import] File: ${latestFile.name}, Status: ${result.summary?.dismissalRisk || "ok"}`);
    ...
    console.error(`[Test Import Error] ${message}`);
